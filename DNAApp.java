 ...
 *
 * Date: 10/07/2018
 * Latest Modified Date: 10/14/2018
 * @author: Joseph Chang
 */
...
public class DNAApp
{
    ...
    //searchAndStore() is similar to searchAndPrint(PrintWriter file)
    //The only difference between them is searchAndStore do not print
    //anything in terminal or file, it store the sequences into a
    // ArrayList<String>.
    public ArrayList<String> searchAndStore()
    {
        int head = searchHeader(0); //head is a index move inside DNA to locate header
        ...
        while (head != -2) //-2 means there is not next header
        {
            sequences.add(DNA.substring(head,head+headerLength));

            int subHead = head+headerLength; //subHead is a head move inside a section
            String codingRegion = searchCodingRegion(subHead);
            while (!codingRegion.equals("End of search in this section"))
            {
                sequences.add(codingRegion);
                subHead = resetSubHead;
                codingRegion = searchCodingRegion(subHead);
            }
